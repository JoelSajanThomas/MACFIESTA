...
export function GalleryModule() {
  ...
  const handleAddItem = (e: React.FormEvent) => {
    ...
    addItem({
      ...
      url: itemUrl,
      thumbnailUrl: itemThumbUrl || (itemType === "image" ? itemUrl : "https://images.unsplash.com/photo-1492684223066-81342ee5ff30?q=80&w=800"),
      featured: isFeatured,
    });

    triggerToast(`✓ New ${itemType === "image" ? "Photo" : "Video"} Added & Synchronized Live!`);
  ...
  return (
    <div className="space-y-6 select-none font-mono">
      ...
      <div className="glass p-6 rounded-3xl border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0A0D1A]">
        <div>
          ...
          <p className="text-xs text-white/50">
            Upload photos and videos separately, toggle featured spotlights, and publish live to public website.
          </p>
        </div>
      ...
      {/* MEDIA ASSETS MASONRY CARDS GRID */}
            ...
            <form onSubmit={handleAddItem} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block text-white/70 font-bold mb-1">
                  {itemType === "image" ? "Photo Image URL" : "Video MP4 / Direct Stream URL"}
                </label>
                <input
                  type="text"
                  placeholder={itemType === "image" ? "https://images.unsplash.com/..." : "https://domain.com/video.mp4"}
                  value={itemUrl}
              ...
              {itemType === "video" && (
                <div>
                  <label className="block text-white/70 font-bold mb-1">Video Cover Thumbnail Image URL (Optional)</label>
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                />
                <label htmlFor="featCheck" className="text-white/80 font-bold cursor-pointer">
                  Feature this asset on Public Homepage Gallery Preview
                </label>
              </div>
