...
async function seedDatabase() {
    try {
        ...
        if (eventCount === 0) {
            ...
            seededEvents = await Event_1.Event.insertMany(eventsToSeed);
            console.log(`Seeded ${seededEvents.length} events successfully.`);
        }
        else {
            seededEvents = await Event_1.Event.find();
        }
        // 2. Seed Scoreboards
        const scoreCount = await Score_1.Score.countDocuments();
        if (scoreCount === 0) {
            ...
            if (scoresToSeed.length > 0) {
                await Score_1.Score.insertMany(scoresToSeed);
